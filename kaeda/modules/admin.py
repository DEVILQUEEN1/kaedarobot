...
from kaeda import dispatcher, WHITELIST_USERS, SUPPORT_USERS, SUDO_USERS, DEV_USERS, OWNER_ID, TOKEN
from kaeda.__main__ import IMPORTED, HELPABLE, MIGRATEABLE, STATS, USER_INFO, DATA_IMPORT, DATA_EXPORT, CHAT_SETTINGS, USER_SETTINGS 
from kaeda.modules.disable import DisableAbleCommandHandler
from kaeda.modules.log_channel import loggable, gloggable
from kaeda.modules.helper_funcs.chat_status import bot_admin, can_promote, user_admin, can_pin, sudo_plus, dev_plus, connection_status
from kaeda.modules.helper_funcs.extraction import extract_user, extract_user_and_text
...
def load(bot: Bot, update: Update):
    ...
    try:
        imported_module = importlib.import_module("kaeda.modules." + text)
    except:
...
def listmodules(bot: Bot, update: Update):
    ...
    for helpable_module in HELPABLE:
        ...
        file_name = file_info.__name__.rsplit("kaeda.modules.", 1)[1]
